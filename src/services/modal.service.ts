...
export class ModalService {
    ...
    private render(id: string, type: Type<any>) {
        if (this._view && type && typeof type !== 'string') {
            ...
            const modal = this._view.createComponent(factory);
            this.modalRef[id] = modal;
                // let's inject @Inputs to component instance
            this.modal[id] = modal.instance;
            this.modal[id].id = id;
            ...
            this._view = this.default_vc;
            if (this.default_vc) {
                return this.render(id, type);
            }
        }
        WIDGETS.error('MODAL(S)', 'No view to render modals on, or not modal component is defined.');
        return { status: () => null };
    }
