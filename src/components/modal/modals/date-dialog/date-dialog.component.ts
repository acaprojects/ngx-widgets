...
export class DateDialog extends Modal {
    ...
    @Input() public color: string = 'teal';
    ...
    public display_year: number = 2016;
    public display_month: number = 0;
    public date_year: string = '2016';
    public date_display: string = 'Today';
    public days: any[] = [];
    public months: any[] = [];
    ...
    public selectDate(week: number, day: number) {
        if (!this.ready) {
            return false;
        }
        if (!+this.month_node[week * 7 + day].valid) {
            return false;
        }
        const date = new Date(this.display_year, this.display_month, +this.month_node[week * 7 + day].day);
        if (this.isBeforeMinDate(+this.month_node[week * 7 + day].day)) {
            return false;
        }
    ...
    public setParams(data: any) {
        ...
        if (data && data.data && data.data.date && data.data.date instanceof Date) {
            this.setDate(data.data.date);
        }
        this.canClose = true;
    }
    ...
    private generateMonth() {
        ...
        for (i = 0; i < monthDays; i++) {
            ...
            this.month_node.push(month_day);
        }
