...
export class InteractiveMap {
    ...
    private map_shown: boolean = false;
    ...
    private next_center: any = { x: 0.5, y: 0.5 };
    private min_center: any = { x: 0.5, y: 0.5 };
    private max_center: any = { x: 0.5, y: 0.5 };
    private marker_list: any[] = [];
    private reset_markers: boolean = false;

    private box_update: any = null;
    ...
    private can_change_zoom: boolean = true;
    private zoom_timer: any = null;

    private updateAnimation: any;
    ...
    constructor(private a: Animate, private service: MapService, private renderer: Renderer) {
    ...
    public update() {
        ...
        const p_z_state = this.zoom_state;
    ...
    public checkValues(force: boolean = false) {
        if (!this.isFocus || force) {
            ...
            if (!this.focus) {
                if (this.center.x < this.min_center.x) {
                    this.center.x = this.min_center.x;
                } else if (this.center.x > this.max_center.x) {
                    this.center.x = this.max_center.x;
                }

                if (this.center.y < this.min_center.y) {
                    this.center.y = this.min_center.y;
                } else if (this.center.y > this.max_center.y) {
                    this.center.y = this.max_center.y;
                }
            } else {
                if (this.center.x < 0) {
                    this.center.x = 0;
    ...
    public tapMap(event: any) {
        ...
        if (event && this.map_display) {
            ...
            if (WIDGETS.get('debug')) {
                ...
                WIDGETS.log('MAP(C)', `Tapped: ${left}, ${top}(${posX.toFixed(0)}, ${posY.toFixed(0)})`);
            }
        }
    ...
    private render() {
        ...
        if (this.map_item && this.active) {
            ...
            if (this.content_box && this.map_item) {
                ...
                if (mbb) {
                    const top = Math.round(-mbb.height * this.center.y + this.content_box.height / 2);
                    const left = Math.round(-mbb.width * this.center.x + this.content_box.width / 2);
                    const x = -(this.center.x * 100);
                    const y = -(this.center.y * 100);
    ...
    private updateMapDetails() {
        ...
        this.map_details.dim.x = Math.ceil(dir ? this.mapSize : (this.mapSize * (mbb.width / mbb.height)));
        this.map_details.dim.y = Math.ceil(!dir ? this.mapSize : (this.mapSize * (mbb.height / mbb.width)));
            // Real Dimensions
     ...
     * @return {void}
     */
    private focusOnPoint(x: number, y: number) {
        if (!this.map_item) {
            return;
        }
        const mbb = this.map_item.getBoundingClientRect();
     ...
     * @return {void}
     */
    private updatePosition(xp: number, yp: number) {
        this.center.x += xp;
        this.center.y += yp;
        this.checkValues();
    }
     ...
     * @return {void}
     */
    private scaleMap(event: any) {
        const scale = event.scale - this.dZoom;
    ...
    private resize() {
        ...
        if (this.map_item && this.map_display) {
            const rect = this.map_item.getBoundingClientRect();
            const md = this.map_display.nativeElement;
        }
        this.updateBoxes();
        this.updateFocus();
        this.loading = false;
